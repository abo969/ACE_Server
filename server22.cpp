/************************************************************************
 ...
 1. if timeout for server waiting, it will start another task, and in the new task,
    it will register a new handler, then the action of registeration will wakeup
    the wait-state thread. then, it will exit from the forever-loop handle_events
    and server will end.
...
{
    ACE_DEBUG ((LM_INFO, "(%t) MyTask2 task start\n\n"));    
