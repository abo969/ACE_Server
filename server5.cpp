/************************************************************************
 ...
 Target:
 1. do not use select, use ACE_SOCK_Dgram method to receive data, if timeout for
    server waiting, it will exit from the forever-loop svc and server will end
...
int MyTask::svc()
{
    ...
    while (1)
    {
        ...
        if (result == -1 && ETIME == errno)	//timeout
        {
            ACE_DEBUG ((LM_INFO, ACE_TEXT ("(%t) recv from socket timeout (%d seconds), result = %d\n\n"), 
				timeout, result));
            break;
        }
