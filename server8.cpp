/************************************************************************
 ...
 1. the target is to verify whether it will call notify method when registering
    or removing socket handlers in another task when the current task handle_events
...
int MyTask2::svc()
{
    ...
    //here, the parameter can not be UDP_PORT, otherwise, 
    //error about "ACE_SOCK_Dgram: Address already in use" and "Invalid argument" will occur
    Handler handler(UDP_PORT + 1000);

    for (int i = 0; i < REGISTER_COUNT; i++)
    {
        ACE_DEBUG ((LM_INFO, ACE_TEXT ("(%t) MyTask2::svc() register user-defined read handler\n")));
        if (ACE_Reactor::instance()->register_handler(&handler, ACE_Event_Handler::READ_MASK) == -1)
        {
            ACE_ERROR((LM_ERROR, "%p\n", "(%t) MyTask2::svc() register user-defined read handler failed\n"));
            return -1;
        }

        ACE_OS::sleep(1);

        ACE_DEBUG ((LM_INFO, ACE_TEXT ("(%t) MyTask2::svc() remove user-defined read handler\n\n")));
        if (ACE_Reactor::instance()->remove_handler(&handler, ACE_Event_Handler::READ_MASK) == -1)
        {
            ACE_ERROR((LM_ERROR, "%p\n", "(%t) MyTask2::svc() remove user-defined read handler failed\n"));
            return -1;
        }
