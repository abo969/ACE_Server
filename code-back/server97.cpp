...
MyTask::~MyTask()
{
    ...
    if (handler_)
    {
        delete handler_;
        handler_ = NULL;
        ACE_DEBUG ((LM_INFO, "(%t) in MyTask::~MyTask() 4\n"));
    }
}
...
int MyTask::close (u_long)
{
    ...
    if (Reactor)
    {
        delete Reactor;
        Reactor = NULL;
        ACE_DEBUG ((LM_INFO, "(%t) in MyTask::close() 2\n"));
    }
    ...
    if (Selector)
    {
        delete Selector;
        Selector = NULL;
        ACE_DEBUG ((LM_INFO, "(%t) in MyTask::close() 3\n"));
    }
    ...
    if (handler_)
    {
        delete handler_;
        handler_ = NULL;
        ACE_DEBUG ((LM_INFO, "(%t) in MyTask::close() 4\n"));
    }
...
int MyTask::svc()
{
    ...
    //these initializations must be placed here (a new thread), must not in its parent thread
    Selector = new ACE_Select_Reactor;
    Reactor = new ACE_Reactor(Selector);
    handler_ = new Handler(UDP_PORT);
    ...
    if (Reactor->register_handler(handler_, ACE_Event_Handler::READ_MASK) == -1)
    {
    ...
    while (exit_flag == 0)
    {
        ...
        if (result)
        {
            ...
            if (RECV_COUNT == handler_->get_count())
            {
                ACE_DEBUG ((LM_INFO, ACE_TEXT ("(%t) MyTask::svc() notify the exit handler\n")));
                ACE_Reactor::instance()->notify(handler_, ACE_Event_Handler::EXCEPT_MASK);
            }

            if (handler_->get_flag())
                exit_flag = 1;
        }
        else
        {
    ...
    if (Reactor->remove_handler(handler_, ACE_Event_Handler::READ_MASK | 
