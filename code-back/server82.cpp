/************************************************************************
 ...
 1. the target is to verify whether it will call notify method when registering
    or removing signal handlers in another task when the current task handle_events has
    no timeout, that is, timeout is NULL. In task1, it will spawn task2 before 
    entering svc-loop, and in the new task, it will register a new signal handler and 
...
public:
    int handle_signal(int signum, siginfo_t * = 0, ucontext_t * = 0)
    {
        if (signum == SIGINT)
        {
            ACE_DEBUG ((LM_INFO, ACE_TEXT ("(%t) handle SIGINT (signum = %d) signal, program end immediately\n"), signum));
        }
        return 0;
    }
};
...
int MyTask2::svc()
{
    ...
    //here, signal handler will be registered to test whether it will invoke notify
    SigintHandler handler;

    for (int i = 0; i < REGISTER_COUNT; i++)
    {
        ACE_DEBUG ((LM_INFO, ACE_TEXT ("(%t) MyTask2::svc() register signal handler\n")));
        if (ACE_Reactor::instance()->register_handler(SIGINT, &handler) == -1)
        {
            ACE_ERROR((LM_ERROR, "%p\n", "(%t) MyTask2::svc() register signal handler failed\n"));
            return -1;
        }

        ACE_OS::sleep(1);

        ACE_DEBUG ((LM_INFO, ACE_TEXT ("(%t) MyTask2::svc() remove signal handler\n\n")));
        if (ACE_Reactor::instance()->remove_handler(SIGINT, NULL, NULL, 0) == -1)
        {
            ACE_ERROR((LM_ERROR, "%p\n", "(%t) MyTask2::svc() remove signal handler failed\n"));
            return -1;
        }
